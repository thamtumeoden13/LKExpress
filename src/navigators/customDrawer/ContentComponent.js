            ...
            <View style={styles.screenContainer}>
                ...
                <TouchableOpacity onPress={() => navigateToScreen('Category')}>
                    <View style={[styles.screenStyle, (state.activeItemKey == 'Category') ? styles.activeBackgroundColor : null]}>
                        <Icon name="sort" type="material" size={scale(22)} color={(state.activeItemKey == 'Category') ? '#00adff' : '#000'} />
                        <Text style={[styles.screenTextStyle, (state.activeItemKey == 'Category') ? styles.selectedTextStyle : null]}>{`Danh mục`}</Text>
                    </View>
                </TouchableOpacity>
                <TouchableOpacity onPress={() => navigateToScreen('Saved')}>
                    <View style={[styles.screenStyle, (state.activeItemKey == 'Saved') ? styles.activeBackgroundColor : null]}>
                        <Icon name="favorite-border" type="material" size={scale(22)} color={(state.activeItemKey == 'Saved') ? '#00adff' : '#000'} />
                        <Text style={[styles.screenTextStyle, (state.activeItemKey == 'Saved') ? styles.selectedTextStyle : null]}>{`Đánh dấu`}</Text>
                    </View>
                </TouchableOpacity>
            </View>
        </Animated.View>
